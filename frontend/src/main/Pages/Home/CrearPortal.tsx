...
interface FormState {
    ...
    descripcion: string;
    tipoAcceso: "ABIERTO" | "CERRADO" | null;
}
...
export function CreatePortal() {
    ...
    const [form, setForm] = useState<FormState>({
        carrera: "", universidad: "", unidadAcademica: "", descripcion: "",
        tipoAcceso: null, // empty state — usuario debe elegir
    });
    const [modoVisual, setModoVisual]               = useState<ModoVisual>("icono");
    const [iconoSeleccionado, setIconoSeleccionado] = useState<string | null>(null); // null = no elegido
    const [colorSeleccionado, setColorSeleccionado] = useState<string | null>(null); // null = no elegido
    ...
    const handleHexInput = (raw: string) => {
        ...
        else { if (colorSeleccionado && clean.length > 0) setHexError(true); }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await portalService.crearPortal({
                ...
                tipoAcceso: form.tipoAcceso ?? "CERRADO",
                iconoPortal: modoVisual === "icono" ? (iconoSeleccionado ?? undefined) : undefined,
                colorPortal: modoVisual === "icono" ? (colorSeleccionado ?? undefined) : undefined,
                logoUrl: undefined,
            });
    ...
    // Preview: solo mostrar valores si el usuario los eligió
    ...
    // Progreso: arrancar desde 0 real
    const accesoElegido  = form.tipoAcceso !== null;
    const visualElegida  = (modoVisual === "icono" && (!!iconoSeleccionado || !!colorSeleccionado))
                         || (modoVisual === "imagen" && !!imagenPreviewUrl);
    const filledCount = [
        !!form.carrera.trim(),
    ...
    // Chip de acceso en preview — solo si eligió
    ...
    return (
            ...
            <div className="f-header-inner max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="f-header-grid">
                    ...
                    <div className="f-copy">
                        <p className="f-eyebrow">nueva comunidad académica</p>
                    ...
                    <div className="f-preview-wrap" aria-label="Vista previa en tiempo real">
                        <p className="f-preview-label">Vista previa</p>

                        <div className="f-preview-card">
                            <div className="f-preview-stripe"
                                style={{ background: previewColor ?? "var(--c-border)" }} />
                            <div className="f-preview-body">
                                <div className="f-preview-avatar"
                                    style={{
                                        background: previewColor ? `${previewColor}15` : "rgba(26,37,48,.06)",
                                        borderColor: previewColor ? `${previewColor}25` : "rgba(26,37,48,.1)",
                                    }}>
                                    <PortalAvatar
                            ...
                            <div className="f-preview-foot">
                                {form.tipoAcceso !== null ? (
                                    ...
                                    <span className="f-access-chip f-access-chip--empty">
                                        Tipo de acceso sin definir
                                    </span>
                                )}
                            </div>
                        </div>
        ...
        <div className="f-body max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                ...
                <section className="f-section">
                    ...
                    <div className="f-section-card f-section-card--I">
                        <div className="f-section-head">
                            ...
                            <p className="f-section-sub">Los campos con <span className="f-req">*</span> son obligatorios.</p>
                        </div>
                        ...
                        <div className="f-fields">
                            <div className="f-field">
                                ...
                                <div className="f-input-wrap">
                                    ...
                                    />
                                    {form.carrera.trim() && (
                                        <span className="f-input-tick"><Check className="w-3.5 h-3.5" /></span>
                                    )}
                                </div>
                            </div>
                                ...
                                <div className="f-color-block">
                                    <div className="f-color-header">
                                        ...
                                        <div className="f-color-current">
                                            <div className="f-color-current-dot"
                                                style={{ background: colorSeleccionado ?? "transparent",
                                                    border: colorSeleccionado ? "none" : "1.5px dashed #8fa8b5" }} />
                                            <code className="f-color-current-code">
                                                {colorSeleccionado ? `#${hexInput.toUpperCase()}` : "Sin elegir"}
                                            </code>
                                        </div>
                                    </div>
                                    ...
                                    <div className="f-hex-row">
                                        <div className="f-hex-preview"
                                            style={{
                                                background: colorSeleccionado ?? "transparent",
                                                border: colorSeleccionado ? "none" : "1.5px dashed #8fa8b5",
                                            }} />
                                        ...
                                        <label className="f-eyedrop" title="Abrir selector del sistema"
                                            ...
                                            <input ref={colorInputRef} type="color"
                                                value={colorSeleccionado ?? "#2c4456"}
                                                ...
                                                <div className="f-icon-grid">
                                                    {resultadosBusqueda.map(({ value, label, Icon }) => (
                                                        <IconBtn key={value} value={value} label={label} Icon={Icon}
                                                            ...
                                                            color={colorSeleccionado ?? "#2c4456"}
                                                            onSelect={setIconoSeleccionado} />
                                                    ))}
                                                </div>
                                            ) : (
                                ...
                                {imagenPreviewUrl ? (
                                    <div className="f-img-row">
                                        <div className="f-img-frame"
                                            style={{ borderColor: colorSeleccionado ? `${colorSeleccionado}50` : "rgba(26,37,48,.15)" }}>
            ...
            {/* ── SIDEBAR — solo checklist, sin card duplicada ── */}
            <aside className="f-sidebar" aria-label="Progreso">
                    ...
                    <div className="f-sb-checklist">
                        ...
                        <ul>
                            {[
                                ...
                                { label: "Tipo de acceso",        done: accesoElegido },
                                { label: "Identidad visual",      done: visualElegida },
...
.cp-root, .f-header, .f-body {
    ...
    --sh-card:      0 1px 3px rgba(26,37,48,.07), 0 4px 18px rgba(26,37,48,.08), 0 0 0 1px rgba(26,37,48,.055);
    --sh-lift:      0 4px 24px rgba(26,37,48,.15), 0 0 0 1px rgba(26,37,48,.07);
    --r:            .85rem;
    --r-sm:         .45rem;
    --r-pill:       999px;
    font-family: 'Inter', system-ui, sans-serif;
}
.cp-root { background: var(--c-paper); min-height: 100vh; }
...
.f-ruled {
    position: absolute; inset: 0; pointer-events: none;
    background-image: repeating-linear-gradient(to bottom, transparent 0px, transparent 31px, var(--c-ruled) 31px, var(--c-ruled) 32px);
}
.f-margin {
    position: absolute; top: 0; bottom: 0; left: 88px;
    width: 1.5px; background: var(--c-margin); pointer-events: none;
}
.f-ink {
...
.f-ink-a {
    width: 360px; height: 280px; top: -120px; right: 8%;
    background: radial-gradient(ellipse, rgba(184,125,18,.11) 0%, transparent 70%);
}
.f-ink-b {
    width: 280px; height: 180px; bottom: -70px; left: -40px;
    background: radial-gradient(ellipse, rgba(44,68,86,.09) 0%, transparent 70%);
}

.f-header-inner {
    position: relative; display: flex; flex-direction: column; padding-top: 2.1rem;
}
.f-back {
    display: inline-flex; align-items: center; gap: .4rem;
    font-size: .71rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase;
    color: var(--c-ink-ghost); background: none; border: none; cursor: pointer; padding: 0;
    margin-bottom: 1.8rem; width: fit-content; transition: color .14s;
}
...
/* Columna copy — centrado verticalmente con align-items: center en el grid */
.f-copy { display: flex; flex-direction: column; justify-content: center; }

.f-eyebrow {
    font-size: .68rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase;
    color: var(--c-ink-ghost); margin: 0 0 .85rem;
}

.f-title {
    font-family: 'Georgia', 'Times New Roman', serif;
    line-height: 1.0; letter-spacing: -.025em; margin: 0 0 1rem; color: var(--c-ink);
}
.f-title-verb {
    display: block; font-size: clamp(2rem, 4.5vw, 3rem);
    font-weight: 400; color: var(--c-ink-mid);
}
.f-title-noun {
    display: block; font-size: clamp(3rem, 7vw, 5rem);
    font-weight: 700; font-style: italic; color: var(--c-navy);
    text-shadow: 2px 2px 0 rgba(26,37,48,.06), 4px 4px 12px rgba(26,37,48,.05);
}

.f-subtitle {
    font-size: .95rem; color: var(--c-ink-soft);
    line-height: 1.75; margin: 0; max-width: 410px;
}

/* Preview card */
.f-preview-wrap { display: flex; flex-direction: column; gap: .55rem; }
.f-preview-label {
...
.f-preview-card {
    ...
    border: 1px solid var(--c-border); transition: box-shadow .2s;
}
.f-preview-card:hover { box-shadow: var(--sh-lift); }
.f-preview-stripe { height: 4px; transition: background .3s; }
.f-preview-body { display: flex; align-items: center; gap: .85rem; padding: .9rem 1rem; }
.f-preview-avatar {
...
.f-preview-name {
    ...
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin: 0 0 .08rem;
}
.f-preview-uni {
...
.f-access-chip {
    ...
    font-size: .67rem; font-weight: 700; letter-spacing: .03em; transition: all .2s;
}
...
.f-access-chip--empty  { background: rgba(26,37,48,.05); color: var(--c-ink-ghost); border: 1px solid rgba(26,37,48,.1); font-style: italic; font-weight: 500; }

/* Barra de progreso */
.f-progress { display: flex; align-items: center; gap: .85rem; padding: 0 0 .5rem; margin-top: .25rem; }
.f-progress-bar { flex: 1; height: 2.5px; background: rgba(26,37,48,.1); border-radius: 3px; overflow: hidden; }
.f-progress-fill {
    height: 100%;
...
.f-progress-label {
    ...
    text-transform: uppercase; color: var(--c-ink-ghost); white-space: nowrap; flex-shrink: 0;
}
...
/* ══ SECCIONES ══ */
.f-section {
    display: flex; gap: 1.1rem; margin-bottom: 1.75rem;
...
.f-section-aside {
    ...
    align-items: center; width: 32px; flex-shrink: 0; padding-top: 1.1rem;
}
.f-aside-num {
...
.f-aside-rule { flex: 1; width: 1.5px; background: var(--c-border); margin-top: .4rem; border-radius: 2px; }
.f-section:last-of-type .f-aside-rule { display: none; }

.f-section-card {
    flex: 1; background: var(--c-surface);
    border-radius: var(--r); box-shadow: var(--sh-card); overflow: hidden;
}
...
.f-section-head { padding: 1.1rem 1.4rem .85rem; border-bottom: 1px solid rgba(26,37,48,.055); }
.f-section-title {
...
/* ══ CAMPOS ══ */
.f-fields { display: flex; flex-direction: column; gap: 1.15rem; padding: 1.25rem 1.4rem 1.4rem; }
.f-row-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 1.15rem; }
...
.f-input:focus { border-color: var(--c-navy-mid); box-shadow: 0 0 0 3px rgba(44,68,86,.11); background: var(--c-surface); }
...
.f-charcount { position: absolute; bottom: .52rem; right: .75rem; font-size: .67rem; color: var(--c-ink-ghost); pointer-events: none; }
...
.f-error-box {
    ...
    border-radius: var(--r-sm); font-size: .82rem; font-weight: 600; color: #991b1b; line-height: 1.5;
}

/* ══ ACCESO ══ */
.f-acceso-grid { display: grid; grid-template-columns: 1fr 1fr; gap: .85rem; padding: 1.1rem 1.4rem 1.35rem; }
@media (max-width: 540px) { .f-acceso-grid { grid-template-columns: 1fr; } }
.f-acceso-card {
    position: relative; display: flex; flex-direction: column;
    align-items: flex-start; gap: .6rem; padding: 1.1rem 1rem;
    border: 1.5px solid var(--c-border); border-radius: var(--r);
    background: var(--c-surface-2); cursor: pointer; text-align: left;
    transition: border-color .14s, background .14s, box-shadow .14s, transform .14s;
}
.f-acceso-card:hover { border-color: var(--c-border-mid); transform: translateY(-1px); box-shadow: var(--sh-card); }
.f-acceso-card--on { transform: translateY(-2px); }
.f-acceso-card--cerrado.f-acceso-card--on { border-color: var(--c-amber); background: rgba(184,125,18,.04); box-shadow: 0 0 0 3px rgba(184,125,18,.1), var(--sh-card); }
.f-acceso-card--abierto.f-acceso-card--on { border-color: var(--c-green); background: rgba(13,110,80,.04); box-shadow: 0 0 0 3px rgba(13,110,80,.1), var(--sh-card); }
.f-acceso-tick {
...
.f-acceso-name { font-size: .9rem; font-weight: 700; color: var(--c-ink); display: flex; align-items: center; gap: .5rem; flex-wrap: wrap; }
.f-rec-pill {
...
/* ══ VISUAL ══ */
.f-tabs {
    display: flex; gap: 2px; background: rgba(26,37,48,.05);
    border-radius: var(--r-sm); padding: 3px; margin: 0 1.4rem .1rem;
}
.f-tab {
...
.f-tab--on { background: var(--c-surface); color: var(--c-ink); box-shadow: 0 1px 4px rgba(26,37,48,.1); }
.f-visual-panel { display: flex; flex-direction: column; gap: 1.15rem; padding: 1rem 1.4rem 1.4rem; }

/* Color picker */
.f-color-block { display: flex; flex-direction: column; gap: .85rem; }
.f-color-header { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
.f-color-current {
    display: flex; align-items: center; gap: .5rem;
    background: var(--c-surface-2); border: 1.5px solid var(--c-border);
    border-radius: var(--r-pill); padding: .28rem .65rem .28rem .38rem;
}
.f-color-current-dot {
    width: 20px; height: 20px; border-radius: 50%; flex-shrink: 0;
    box-shadow: 0 1px 4px rgba(0,0,0,.15); transition: background .25s;
}
...
.f-palette { display: grid; grid-template-columns: repeat(6, 1fr); gap: .5rem; }
.f-swatch {
    position: relative; aspect-ratio: 1; border-radius: .6rem;
    border: 2.5px solid transparent; cursor: pointer; overflow: hidden;
    transition: transform .12s cubic-bezier(.16,1,.3,1), box-shadow .12s, border-color .12s;
    box-shadow: 0 1px 4px rgba(0,0,0,.2), inset 0 1px 0 rgba(255,255,255,.18);
}
@media (hover: hover) and (pointer: fine) {
    .f-swatch:hover {
        transform: scale(1.18) translateY(-2px);
        box-shadow: 0 6px 18px rgba(0,0,0,.3), inset 0 1px 0 rgba(255,255,255,.2);
        z-index: 2;
    }
}
.f-swatch--on {
    border-color: rgba(255,255,255,.9);
    box-shadow: 0 0 0 2.5px rgba(0,0,0,.55), 0 4px 14px rgba(0,0,0,.3);
    transform: scale(1.1) translateY(-1px); z-index: 3;
}
.f-swatch-fill { display: block; width: 100%; height: 100%; border-radius: calc(.6rem - 2.5px); }
.f-swatch-check {
...
.f-hex-preview {
    ...
    box-shadow: 0 2px 8px rgba(0,0,0,.15); transition: background .25s;
}
.f-hex-field {
    flex: 1; display: flex; align-items: center;
    background: var(--c-surface-2); border: 1.5px solid var(--c-border);
    border-radius: var(--r-sm); overflow: hidden; transition: border-color .13s;
}
.f-hex-field:focus-within { border-color: var(--c-navy-mid); box-shadow: 0 0 0 3px rgba(44,68,86,.1); }
...
.f-eyedrop {
    ...
    background: var(--c-surface-2); color: var(--c-ink-soft); cursor: pointer;
    transition: border-color .13s, color .13s, background .13s;
}
.f-eyedrop:hover { border-color: var(--c-navy-mid); color: var(--c-navy-mid); background: rgba(44,68,86,.06); }
.f-hex-err { font-size: .71rem; color: var(--c-red); margin: 0; }

.f-divider { display: flex; align-items: center; gap: .7rem; }
.f-divider::before, .f-divider::after { content: ''; flex: 1; height: 1px; background: var(--c-border); }
.f-divider span { font-size: .66rem; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; color: var(--c-ink-ghost); white-space: nowrap; }

/* Icon browser */
.f-icon-block { display: flex; flex-direction: column; gap: .6rem; }
.f-search-wrap { position: relative; display: flex; align-items: center; }
.f-search-icon { position: absolute; left: .75rem; width: 14px; height: 14px; color: var(--c-ink-soft); pointer-events: none; }
.f-search-input {
    width: 100%; padding: .6rem .9rem .6rem 2.1rem; font-size: .86rem; font-family: inherit;
    background: var(--c-surface-2); border: 1.5px solid var(--c-border);
    border-radius: var(--r-sm); color: var(--c-ink); outline: none;
    transition: border-color .13s, box-shadow .13s;
}
.f-search-input:focus { border-color: var(--c-navy-mid); box-shadow: 0 0 0 3px rgba(44,68,86,.1); }
.f-search-clear { position: absolute; right: .6rem; background: none; border: none; cursor: pointer; color: var(--c-ink-soft); display: flex; transition: color .13s; }
.f-search-clear:hover { color: var(--c-ink); }
.f-icon-browser {
    max-height: 288px; overflow-y: auto; border: 1.5px solid var(--c-border);
    border-radius: var(--r-sm); padding: .7rem; background: var(--c-surface-2);
    scrollbar-width: thin; scrollbar-color: rgba(26,37,48,.18) transparent;
}
.f-icon-cat { margin-bottom: .8rem; }
.f-icon-cat:last-child { margin-bottom: 0; }
.f-icon-cat-label { font-size: .61rem; font-weight: 800; text-transform: uppercase; letter-spacing: .1em; color: var(--c-ink-ghost); margin: 0 0 .38rem; }
.f-icon-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(54px, 1fr)); gap: .3rem; }
.f-icon-empty { display: flex; flex-direction: column; align-items: center; gap: .5rem; padding: 2rem; color: var(--c-ink-soft); font-size: .85rem; text-align: center; }

/* Imagen */
.f-upload-zone {
    display: flex; flex-direction: column; align-items: center; gap: .6rem;
    width: 100%; padding: 2.5rem 1rem; border: 2px dashed var(--c-border-mid);
    border-radius: var(--r); background: var(--c-surface-2); cursor: pointer;
...
.f-img-frame { width: 72px; height: 72px; border-radius: .6rem; border: 2px solid; overflow: hidden; flex-shrink: 0; transition: border-color .25s; }
...
.f-img-name { font-size: .82rem; font-weight: 600; color: var(--c-ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.f-img-remove {
    display: inline-flex; align-items: center; gap: .35rem; padding: .3rem .62rem;
    font-size: .74rem; font-weight: 600; color: #991b1b;
    background: rgba(185,28,28,.07); border: 1px solid rgba(185,28,28,.14);
    border-radius: var(--r-pill); cursor: pointer; transition: background .13s;
}
...
.f-btn-submit {
    ...
    padding: .88rem 2.1rem; font-size: .92rem; font-weight: 700; letter-spacing: .01em;
    background: var(--c-navy); color: white; border: none; border-radius: var(--r-pill);
    cursor: pointer; box-shadow: 0 2px 14px rgba(26,37,48,.32);
    transition: background .14s, transform .14s, box-shadow .14s;
}
@media (hover: hover) and (pointer: fine) {
    .f-btn-submit:hover:not(:disabled) { background: var(--c-navy-lite); transform: translateY(-2px); box-shadow: 0 7px 22px rgba(26,37,48,.38); }
    .f-btn-submit:active:not(:disabled) { transform: translateY(0); }
}
.f-btn-submit:disabled { opacity: .38; cursor: not-allowed; }
.f-btn-cancel {
    display: inline-flex; align-items: center; padding: .88rem 1.5rem;
    font-size: .88rem; font-weight: 600; color: var(--c-ink-soft);
    background: transparent; border: 1.5px solid var(--c-border); border-radius: var(--r-pill);
...
.f-sidebar-inner { position: sticky; top: 1.5rem; display: flex; flex-direction: column; gap: 1rem; }
.f-sb-checklist {
    background: var(--c-surface); border-radius: var(--r);
...
.f-sb-checklist-title {
    ...
    letter-spacing: .09em; color: var(--c-ink-ghost); margin: 0 0 .7rem;
}
...
.f-chk-dot {
    ...
    display: flex; align-items: center; justify-content: center;
    flex-shrink: 0; color: white; transition: background .15s, border-color .15s;
}
...
.f-sb-note {
    display: flex; align-items: flex-start; gap: .55rem; padding: .68rem .85rem;
    background: rgba(26,37,48,.03); border: 1px solid var(--c-border); border-radius: var(--r-sm);
}
...
.dark .f-input:focus, .dark .f-search-input:focus { border-color: #5d8cc7; box-shadow: 0 0 0 3px rgba(93,140,199,.12); }
...
.dark .f-sb-checklist { border-color: rgba(255,255,255,.07); }
...
@media (prefers-reduced-motion: reduce) {
    .f-section, .f-swatch:hover, .f-btn-submit:hover, .f-acceso-card:hover, .f-acceso-card--on {
...
function IconBtn({
    ...
    return (
        <button
            ...
            onMouseEnter={e => {
                if (!selected) (e.currentTarget as HTMLButtonElement).style.transform = "scale(1.1)";
            }}
            onMouseLeave={e => {
                (e.currentTarget as HTMLButtonElement).style.transform = selected ? "scale(1.05)" : "scale(1)";
            }}
        >
            <Icon style={{ width: 19, height: 19, color: selected ? color : "#6a8898", transition: "color .12s" }} />
            <span style={{
